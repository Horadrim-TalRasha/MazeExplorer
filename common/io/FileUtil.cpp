...
int FileUtil::GetDiskSz7OldTm(char *szpDatPath, unsigned long long &u64CurSize, time_t &tMin, char *szpOldFile)
{
	struct dirent64 *pDirTerm;
...
int FileUtil::RefreshFile(char *szpFileName, unsigned long long &u64AccessTime)
{
	time_t tNow = 0;
	time(&tNow);
	struct stat64 stInfo;
	if(stat64(szpFileName, &stInfo) == 0)
	{
		struct utimbuf tm;
		tm.actime = tNow; //st_atime和st_ctime都会变化
		tm.modtime = stInfo.st_mtime;
		utime(szpFileName, &tm);
		u64AccessTime = tNow;
		return 0;
	}
	return -1;
